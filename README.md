...
Это изменённая версия Chili3D, используемая в сервисе «Макетка» (cad.cmit.ru).
Правообладатель изменений — ООО «Бравый страус». Изменения внесены в 2026 году
и распространяются по той же лицензии AGPL-3.0.

Программа использует Open CASCADE Technology (OCCT) — геометрическое ядро,
распространяемое по LGPL-2.1 с исключением OCCT.

### Сборка боевого образа

Образ, который работает на cad.cmit.ru, собирается из этого репозитория:

```
docker build -f deploy/Dockerfile -t maketka-editor .
docker run --rm -p 8080:80 maketka-editor     # редактор: http://localhost:8080/3d/
```

В `deploy/` лежит всё, что нужно для сборки и установки: `Dockerfile` и конфигурация
веб-сервера `nginx.conf` (маршруты редактора, политика безопасности содержимого,
проверка доступа к работе, отдача исходников по AGPL §13). Это соответствующий
исходный текст в смысле AGPL: лицензия относит к нему и сценарии, управляющие сборкой
и установкой.

В приватном контуре остаются только эксплуатационные значения: какой коммит форка
сейчас на бою (`fork.lock`), адрес зеркала пакетов и секреты. Собрать работающий
редактор можно, ничего оттуда не получая.

Ядро геометрии (`packages/wasm/lib/chili-wasm.wasm`) хранится в репозитории собранным;
пересобрать его из исходников на C++ — `npm run build:wasm` (нужен emscripten, см.
раздел «WASM Build» выше).

### Что изменено относительно upstream

**Безопасность.** Удалены все пути загрузки и исполнения стороннего кода:
менеджер плагинов, параметры `?plugin=`, `?url=`, `?model=`, автозагрузка
`plugins.json`, загрузка иконок через `fetch` + `new Function`, встроенный
трекер Microsoft Clarity. Редактор работает на одном origin с сессией ребёнка,
поэтому любой такой путь означал бы захват детского аккаунта.

**Облачное хранение.** Добавлен `CloudStorage` — реализация `IStorage` поверх
API школьного сервиса: тело работы хранится на сервере, IndexedDB остаётся
буфером несохранённых правок. Запись поверх более свежей ревизии отклоняется
сервером, вместо молчаливой перезаписи предлагается сохранить копию.

**Язык.** Русский словарь переписан под школьника («Вытянуть» вместо
«Экструзия», «Скруглить углы» вместо «Заполнить»), добавлены девять
отсутствовавших ключей, исправлен выбор языка: `navigator.language` вида
`ru-RU` теперь находит словарь `ru`.

**Размещение.** Сборка настроена на путь `/3d/`, имена файлов содержат хэш
содержимого (иначе кэш отдавал бы детям устаревший редактор после обновления).

Экран загрузки показывает ход в процентах и понятную ошибку вместо
системного `alert`.
