...
/**
 * Кадр для кабинета: камера отъезжает так, чтобы модель влезла целиком, снимок,
 * камера обратно.
 *
 * Наводим ВСЕГДА, а не только на первом кадре (B-224, решение владельца
 * 06.09.2026). Без наводки плитка показывала то, куда смотрел ученик, а это
 * сплошь и рядом чёрный квадрат: «Образец финала» во всех четырёх копиях
 * (работы 89, 96, 108, 337) дал байт в байт одинаковый пустой кадр 1,3 КБ.
 * Тем же приёмом снимает ИИ-помощник (`aiOps.ts`).
 *
 * Камеру возвращаем на место: снимок для кабинета не должен дёргать вид под
 * рукой у ученика.
 */
function снимокМодели(app: IApplication): string | undefined {
    ...
    feedbackWindow = new Feedback({
        ...
        shot: async () => {
            ...
            const снимок = await снимокОкна({
                // Кадр холста живёт только до конца такта отрисовки, поэтому
                // сцену рисуем прямо перед тем, как снимать с неё пиксели, —
                // тем же приёмом, что `toImage` и превью работы.
                отрисовать: () => app.activeView?.render(),
