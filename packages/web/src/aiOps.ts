...
//
// ИИ-помощник наставника присылает через оболочку план построения — до 60
...
// состояния: «строит ИИ-помощник, ничего не нажимайте» — и кнопкой «Остановить».
//
...
/** Число для человека: без хвоста из нулей и без длинной дроби. */
function число(v: number): string {
    return String(Math.round(v * 100) / 100);
}

/**
 * Предел толщины стенки для этой фигуры. Стенки растут внутрь от всех граней,
 * кроме снятой верхней: по ширине и глубине их две, по высоте одна — только дно.
 * Если стенка толще предела, ядро молча возвращает исходное тело, и ребёнок не
 * узнаёт, что просьбу не выполнили (B-193). Рамка — оценка сверху: у наклонных
 * и круглых фигур место кончится раньше, и тогда откажет уже ядро.
 */
export function пределСтенки(shape: IShape): number {
    const рамка = shape.boundingBox();
    return Math.min(
        (рамка.max.x - рамка.min.x) / 2,
        (рамка.max.y - рамка.min.y) / 2,
        рамка.max.z - рамка.min.z,
    );
}
...
export class AiOps {
    ...
    private async poll(): Promise<number> {
        ...
        if (data.role === "stale" || batch.dict_version !== DICT_VERSION) {
            this.show("ИИ-помощник прислал задание для обновлённого редактора — обновите страницу (F5).", {
                blocking: false,
                stoppable: false,
            });
            return IDLE_MS;
        ...
        if (data.role !== "executor" || !this.canExecute()) {
            ...
            this.show(
                `Сейчас строит ИИ-помощник (в другой вкладке): шаг ${appliedCount} из ${batch.steps.length}. Пожалуйста, ничего не нажимайте.`,
            );
            return ACTIVE_MS;
        }
        ...
        for (;;) {
            ...
            this.show(
                `Сейчас строит ИИ-помощник: шаг ${step.step_no} из ${batch.steps.length}. Пожалуйста, ничего не нажимайте.`,
            );
            try {
                ...
                if (done) {
                    this.wasActive = false;
                    this.showFinal("ИИ-помощник закончил построение — работа сохранена.");
    ...
    private applyInner(stepNo: number, op: OpData) {
        ...
        switch (kind) {
            ...
            case "полость": {
                ...
                const предел = пределСтенки(shape);
                if (thickness >= предел) {
                    throw new Error(
                        `стенка ${число(thickness)} мм для этой фигуры слишком толстая: ` +
                            `она должна быть тоньше ${число(предел)} мм`,
                    );
                }
