...
//
// Из правки есть выход (B-257): плашка правки несёт кнопку «Выйти из правки»,
// и наставник возвращается в просмотр, не уходя со страницы. Пока он в правке,
// работа заперта для ученика — замок держит `editLock.ts`.
//
// Наставник открывает работу ученика из ленты группы. По умолчанию это
...
export interface ViewBannerOptions {
    ...
    isExample?: boolean;
    /** Есть ли вообще право править (у наставника группы и админа — есть). */
    canEdit: boolean;
    /** Вызывается при переключении в правку: включает автосохранение. */
    onEdit: () => void;
    /** Возврат в просмотр (B-257): досылает несохранённое и гасит автосохранение. */
    onStopEdit: () => Promise<void>;
...
export class ViewBanner {
    ...
    private showViewing() {
        this.root.innerHTML = "";
        // Возврат из правки: цвета плашки тоже возвращаются — зелёная «правлю»
        // и жёлтая «смотрю» различаются издалека, и путать их нельзя.
        this.root.style.background = "#fdf6e3";
        this.root.style.borderColor = "#c98a10";
        this.root.style.color = "#7a5408";
    ...
    private showEditing() {
        ...
        // Дверь наружу (B-257): до неё выйти из правки можно было только уходом
        // со страницы, а ученик всё это время не мог трогать свою работу.
        const stop = this.button("Выйти из правки", false);
        stop.style.borderColor = "#0e7a5f";
        stop.style.color = "#0b3d31";
        stop.onclick = async () => {
            stop.disabled = true;
            stop.textContent = "Сохраняю…";
            await this.options.onStopEdit();
            this.showViewing();
        };
