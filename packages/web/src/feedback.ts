...
export class Feedback {
    ...
    open() {
        ...
        const показать = () => {
            ...
            // `hidden` прячет только то, у чего нет своего `display`: у заглушки он
            // инлайновый (`display: grid`) и перебивает правило браузера. Без этой
            // строки «Готовим картинку экрана…» висит над уже готовым снимком —
            // поймано браузерной проверкой 08.09.2026.
            место.style.display = место.hidden ? "none" : "grid";
