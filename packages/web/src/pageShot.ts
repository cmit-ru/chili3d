...
export interface НастройкиСнимка {
    ...
    /**
     * Перерисовать сцену прямо сейчас. Пиксели 3D-холста живут только до конца
     * такта отрисовки (`preserveDrawingBuffer` выключен), поэтому кадры холстов
     * снимаются в том же такте, что и отрисовка, — тем же приёмом, что
     * `IView.toImage` и превью работы (`preview.ts`). Без этого `toDataURL`
     * молча отдаёт пустой кадр: исключения он не бросает, и запасной путь
...
export async function снимокОкна(настройки: НастройкиСнимка): Promise<Снимок> {
    ...
    try {
        ...
        // Отрисовка — здесь же, в этом же такте: кадр холста живёт только до
        // конца такта отрисовки.
        настройки.отрисовать?.();
        const клон = мёртвыйКлон();
...
/**
 * Копия страницы в отдельном документе — том, у которого нет своего окна, а
 * значит и реестра пользовательских элементов.
 *
 * `document.body.cloneNode(true)` копировал и `<chili3d-main-window>` со всей
 * мастерской внутри, а браузер тут же поднимал клоны — звал их конструкторы,
 * причём без аргументов. `MainWindow` первым делом делает
 * `document.body.appendChild(this)`, и на живую страницу приезжала вторая,
 * недособранная мастерская поверх настоящей (`position:absolute`, во весь
 * экран); `Viewport` и `LayoutViewport` по дороге затирали себе `className`.
 * Отсюда и «модель исчезла», и «нижняя панель ушла под свойства», и мёртвая
 * страница после закрытия окна отзыва: копия оставалась в `body` навсегда.
 *
 * В документе без окна конструкторы не запускаются — копируется только разметка.
 */
function мёртвыйКлон(): HTMLElement {
    const мёртвый = document.implementation.createHTMLDocument("");
    return мёртвый.importNode(document.body, true) as HTMLElement;
}
...
export function подменитьХолсты(клон: HTMLElement, оригинал: HTMLElement): void {
    ...
    Array.from(клон.querySelectorAll("canvas")).forEach((холст, индекс) => {
        ...
        try {
            // Кадр читается уже после отрисовки сцены (см. `отрисовать`): без
            // неё живой 3D-холст отдаёт пустоту и об этом не сообщает.
