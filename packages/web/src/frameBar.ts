...
export class FrameBar {
    ...
    /**
     * Правка встала в очередь на запись. Пока сервер её не подтвердил, слово
     * «Сохранено» — неправда: до B-208 полоса говорила его и сразу после
     * правки, и после перезагрузки, потерявшей эту правку.
     *
     * Тревожные состояния не перебиваем: «Нет интернета» и расхождение версий
     * ребёнку важнее, чем то, что в очереди появилась ещё одна правка.
     */
    markPending() {
        if (this.state === "idle" || this.state === "saved") this.setSaveState("saving");
    }
    ...
    /**
     * Ребёнок (или наставник) нажал «Править» в плашке чужой работы: слово
    ...
    private failureBanner(code?: string) {
        showBanner({
            key: "save-failed",
            text: "Не получается сохранить. Покажи это наставнику",
            hint: `покажи наставнику: работа ${this.options.projectId ?? "—"}, ошибка ${code ?? "нет ответа"}`,
            actions: [
                {
    ...
    private renderUser(host: HTMLElement, user: FrameUser) {
        ...
        // ровно те же слова). Наставник уходит из системы — «Выйти». У ребёнка
        // дверь тоже начинается с «Выйти» (решение владельца 07.09.2026: без
        // этого слова он её не узнаёт), а вторая половина говорит, что на самом
        // деле произойдёт: на общем компьютере класса он уступает место соседу,
        // на своём — отвечает теми же словами, что в верхней строке кабинета.
        const leaveText =
            user.role !== "student"
                ? "Выйти"
                : this.options.sharedPc
                  ? "Выйти / Передать компьютер"
                  : "Выйти / Это не я";
