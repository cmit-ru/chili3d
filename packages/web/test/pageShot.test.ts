...
describe("Снимок окна мастерской", () => {
    ...
    test("клон страницы живёт в отдельном документе, а не в живом", async () => {
        // Клон из живого документа браузер поднимает как пользовательские
        // элементы: конструктор `MainWindow` кладёт копию мастерской обратно в
        // `body`, и страница ломается (B-249). happy-dom про окно документа не
        // знает и конструкторы зовёт всегда, поэтому саму поломку тут не
        // воспроизвести — держим то, на чём лечение стоит: чужой документ.
        let вЖивом: boolean | null = null;
        await снимокОкна({
            запасной: () => КАДР_СЦЕНЫ,
            правкаКлона: (клон) => {
                вЖивом = клон.ownerDocument === document;
            },
        });

        expect(вЖивом).toBe(false);
    });
    ...
    describe("когда браузер умеет рисовать", () => {
        ...
        test("кадр сцены снимают после отрисовки — иначе холст отдаёт пустоту", async () => {
            let отрисовано = false;
            const холст = document.createElement("canvas");
            холст.setAttribute("style", "width:640px;height:480px");
            // Живой 3D-холст без отрисовки в том же такте отдаёт пустой кадр
            // и не бросает исключения — молча, поэтому осечку никто не ловил.
            холст.toDataURL = () => (отрисовано ? КАДР_СЦЕНЫ : "");
            document.body.append(холст);
